...
function setup(input)
{
	...
	xhr1.onreadystatechange = function()
	{
		if(xhr1.status==200 && xhr1.readyState==4)
		{
			...
			{
				if(response[i]['sid']==input[0]['sid'])
					document.getElementById('store').innerHTML+='<option value="'+response[i]['sid']+'" selected>'+response[i]['sname']+'</option>';
				else
...
function editNew()
{
	...
	xhr1.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhr1.send("uid=0&pname="+pname+"&qty="+qty+"&sid="+sid+"&category="+category+"&price="+price+"&description="+description+"&pid="+pid);
}
